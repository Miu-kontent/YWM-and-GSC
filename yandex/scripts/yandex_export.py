...
def main():
    ...
    if filter_links:
        ...
        for link in filter_links:
            ...
            if '://' in link:
                link = link.split('://', 1)[1]
            link = link.rstrip('/')
            filter_normalized.add(link.lower())

        hosts = []
        for h in all_hosts:
            url = h.get("unicode_host_url", "")
            site = extract_site_url(url)
            if site.lower() in filter_normalized:
                hosts.append(h)
    else:
        hosts = all_hosts
    ...
    for host in hosts:
        ...
        site_display = extract_site_url(h_url)
        ...
        if err:
            print(f"⚠️  Ошибка получения данных сайта {hid}: {err}")
            continue

        display_name = details.get("host_display_name", site_display)
